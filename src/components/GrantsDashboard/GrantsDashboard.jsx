      ...
      <div className="filters-section">
        <div className="filters-container">
          ...
          <div className="filters-primary">
            <div className="filter-group filter-group-selectors">
              <UserTypeSelector 
                userType={userType} 
          ...
          <div className="filters-secondary">
            <div className="filter-group filter-group-status">
              ...
              <div className="status-toggles" role="group" aria-label="Status filters">
                {[
                  { key: 'open', label: 'Open' },
            ...
            <div className="filter-group filter-group-favorites">
              <button
                className={`favorite-toggle ${favoriteFilter === 'saved' ? 'active' : ''}`}
                onClick={() => {
                ...
                title={`View saved grants (${favorites.length})`}
                type="button"
              >
                <Heart size={16} fill={favoriteFilter === 'saved' ? 'currentColor' : 'none'} />
                <span>Saved</span>
        ...
        {(searchQuery || userType !== 'all' || selectedDepartment !== 'all' || 
          Object.values(statusFilter).some(v => v) || favoriteFilter === 'saved' || 
          sourceFilters.ca === false || sourceFilters.federal === false) && (
          <div className="active-filters-container" aria-live="polite">
            <div className="active-filters">
              ...
              {userType !== 'all' && (
                <button className="filter-chip" onClick={() => setUserType('all')} title="Remove user filter" type="button">
                  {userType === 'county' ? 'County Department' : 'Community Organization'} <X size={12} />
                </button>
              )}
              ...
              {Object.entries(statusFilter).filter(([_k,v]) => v).map(([k]) => (
                <button key={k} className="filter-chip" onClick={() => setStatusFilter(prev => ({ ...prev, [k]: false }))} title={`Remove ${k} filter`} type="button">
                  Status: {k.charAt(0).toUpperCase() + k.slice(1)} <X size={12} />
                </button>
              ))}
              {favoriteFilter === 'saved' && (
                <button className="filter-chip" onClick={() => setFavoriteFilter('all')} title="Show all grants" type="button">
                  Saved Only <X size={12} />
                </button>
              )}
              {sourceFilters.ca === false && (
                <button className="filter-chip" onClick={() => setSourceFilters(prev => ({ ...prev, ca: true }))} title="Include CA grants" type="button">
                  CA Excluded <X size={12} />
                </button>
              )}
              {sourceFilters.federal === false && (
                <button className="filter-chip" onClick={() => setSourceFilters(prev => ({ ...prev, federal: true }))} title="Include Federal grants" type="button">
                  Federal Excluded <X size={12} />
                </button>
              )}
              <button 
